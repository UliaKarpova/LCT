...
function Monitoring() {
  ...
  return (
    <main className='main'>
      <ManagerPanel title='Ежедневный мониторинг' 
          btnText='Редактировать распределение' 
          placeholderText='Поиск по задачам'
          /*onClick={onAddWorker}*/ />
          <div className='all_tasks'>
            <div className='all_tasks_cell'>
              <span style={{color: '#0B0F14'}} className='number'>{tasks.total_tasks}</span>
              <span className='caption'>Всего задач</span>
            </div>
            <div className='all_tasks_cell'>
              <span style={{color: '#5AA300'}} className='number'>{tasks.done_tasks}</span>
              <span className='caption'>Завершены</span>
            </div>
            <div className='all_tasks_cell'>
              <span style={{color: '#003791'}} className='number'>{tasks.in_progress_tasks}</span>
              <span className='caption'>В работе</span>
            </div>
            <div className='all_tasks_cell'>
              <span style={{color: '#40474F'}} className='number'>{tasks.waiting_tasks}</span>
            ...
            <tbody>
              {ofs.map((of, index) => {
                if (of.employee_info.length > 0) {
                  of.employee_info[0].avatar = index === 0 || index === 3 || index === 6 ? face1
                    : index === 1 || index === 4 || index === 7 ? face2
                    : face3;
                  return (
                    <tr key={of._id} className='of_row'>
                      <td className='of_cell'>{of.employee_info[0].PriorityReason}</td>
                      <td className='of_cell'><img className='cell_avatar' src={of.employee_info[0].avatar} alt='Аватар' />{of.employee_info[0]['ФИО']}</td>
                      <td className='of_cell'>{of.employee_info[0].priority}</td>
                      <td className='of_cell'><img className='cell_img' src={getSrc(of.employee_info[0])} alt='Статус задачи' /></td>
                    </tr>
                  )
                }
              })}
